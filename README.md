...
**Recommended Approach: Use the Cognitive-Modeler Agent**

The easiest way to create new cognitive spaces is to use Claude Code's specialized `cognitive-modeler` agent:

```bash
# Simply ask Claude Code to model your thoughts
# Example: "I want to model the relationship between trust and collaboration"
# The cognitive-modeler agent will automatically:
# - Create the space directory with timestamp ID
# - Generate the TypeScript thought model
# - Execute and validate the space
...
1. **Create Space Directory**
   ```bash
   # Generate unique space ID (timestamp-based)
   SPACE_ID=$(date +%Y-%m-%dT%H-%M-%S-%3NZ)
   mkdir -p data/spaces/$SPACE_ID
   ```

2. **Write Thought Model Script**
   Create `data/spaces/$SPACE_ID/space.ts`:
   ```typescript
   ...
   const space = new Space(
     'your-space-id',
     'Space Title',
     'Brief description of what this space explores'
   );

   // Create your thoughts - IntelliSense guides you through valid options
   space.thought('MyThought')
...
3. **Validate TypeScript (Essential)**
   ```bash
   # Always validate syntax before saving - auto-execution requires valid TypeScript
   npx tsc --noEmit data/spaces/$SPACE_ID/space.ts
   ```

4. **Auto-Execution (Automatic when dashboard is running)**
   When `npm run dev` is running, files are automatically executed on save.

   **Manual execution** (if needed):
   ```bash
   npx tsx execute-space.ts $SPACE_ID
   ```

   Auto-execution will:
   - Detect space.ts file changes via file watcher
   - Validate TypeScript syntax automatically
   - Execute and capture JSON output (only if syntax is valid)
   - Write result to `data/spaces/$SPACE_ID/space.json`
   - Dashboard updates immediately via WebSocket

### **Key Workflow Principles**

- **TypeScript Validation First**: Always validate syntax with `npx tsc --noEmit` before saving - auto-execution depends on valid TypeScript
...
- **Space-based API**: Use the `Space` class from `src/lib/thought-system` for new cognitive models
- **Execution-Based Generation**: JSON files are generated by executing TypeScript, never written directly
- **Space Isolation**: Each space is a separate directory with its own thought models
- **Real-time Updates**: WebSocket server watches for file changes and auto-executes valid TypeScript
...
**Start Here**:
1. **Use the `cognitive-modeler` agent** - Automatically creates spaces with proper structure
2. **[`src/lib/types.ts`](src/lib/types.ts)** - Self-documenting API reference for manual creation
3. **[`docs/examples/simple-space.ts`](docs/examples/simple-space.ts)** - Quick start example
