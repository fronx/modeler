...
2. **Write Thought Model Script**
   Create `data/sessions/$SESSION_ID/model-your-thoughts.ts`:
   ```typescript
   import { ThoughtSpace } from '../../../artifacts/claude-code/thought-system.js';

   const space = new ThoughtSpace();
   const t = (id: string) => space.thought(id);
...
- **Serialization Endpoint**: Every thought model must end with `if (require.main === module) { console.log(JSON.stringify(space.serialize(), null, 2)); }`
...
│   ├── model-our-objectives.ts  # TypeScript thought model (source)
